...
4. Never advance a phase without an explicit commit signal. Dispatch Phases 5 (Critique) and 6 (Plan) as **genuinely isolated sub-agents** per `core/CLAUDE.md` §3 — feed them `core/agents/critic.md` / `core/agents/planner.md` and only `{chosen_idea, scope, constraints}`. Do **not** use the to-do tools (`TaskCreate`/`TaskUpdate`); they don't isolate.
5. After Phase 6, offer `/scaffold` to turn the brainstorm into a ready-to-build folder (see `core/agents/scaffolder.md`).
