...
4. Never advance a phase without an explicit commit signal. Run **Phase 5 (Critique) in-context** following `core/agents/critic.md` (you have the full brainstorm — use it, but don't let attachment soften the verdict, and don't spawn a sub-agent). Dispatch **Phase 6 (Plan) as a genuinely isolated sub-agent** per `core/CLAUDE.md` §3 using `core/agents/planner.md`. Do **not** use the to-do tools (`TaskCreate`/`TaskUpdate`).
