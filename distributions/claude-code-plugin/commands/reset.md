...
1. Read `${CLAUDE_PLUGIN_ROOT}/core/CLAUDE.md` (the memory model: append-only log, one file per project).
2. Confirm the user wants to start a **new** project — don't abandon the current one silently.
3. Create a new `context/<slug>.md` from `${CLAUDE_PLUGIN_ROOT}/core/templates/project-context.md` for the new idea, add a line to `context/INDEX.md`, then begin **Phase 1 — Understanding**.
