...
2. Launch a **genuinely isolated sub-agent** using your environment's sub-agent launch tool — the **`Task`** tool in Claude Code / the **`Agent`** tool in Cowork (not the `TaskCreate`/`TaskUpdate` to-do tools), feeding it `${CLAUDE_PLUGIN_ROOT}/core/agents/planner.md` plus `{chosen_idea, critique_output, constraints}`.
